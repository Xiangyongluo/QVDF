...
def _congested_volume_speed_function(vct, D_C, cd_alpha, cd_beta):
    return vct / (1 + cd_alpha * np.power(D_C, cd_beta))
...
def generateTimeDependentQueue(link_file='link_performance.csv'):
    ...
    with open("tmc_link_performance_" + link_file[:-4] + ".csv", "w", newline="") as performance_fp:
        ...
        time_in_hour = []
        for _ in range(number_of_interval):
