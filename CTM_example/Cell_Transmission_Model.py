...
class Cell(object):
    ...
    def __init__(self, cellid, linkid, zoneid, time_interval=6, k=0, qmax=2160, kjam=220, vf=60, w=12, 
        ...
        if Cell.idcase.get(self.getCompleteAddress()) is None:
    ...
    def addConnection(self, sink):
        if len(sink.cfrom) == 2 or len(self.cto) == 2:
            raise Exception(
                f"Cannot add more connection to cell {self.getCompleteAddress()} and cell {sink.getCompleteAddress()}"
            )
    ...
    def deleteConnection(self, sink):
        if sink not in self.cto:
            raise Exception(
                f"Cell {self.getCompleteAddress()} is not connected with cell {sink.getCompleteAddress()}"
            )
    ...
    def getCell(self):
        return Cell.idcase[self]
    
    def getFirstCell(self):
        newDict = {
            key: Cell.idcase[key]
            for key in Cell.idcase
            if Cell.idcase[key].linkid == self
        }
...
class node(object):
    ...
    def getNodeFromID(self):
        return node.idcase[self]
...
class link(object):
    ...
    def getLinkFromID(self):
        return link.idcase[self]
...
def quicklyCreateCells(number, linkid):
    cells = [
        Cell(f'C{str(i)}', linkid, 'A0', arr_rate=0, dis_rate=1800)
    ...
    for corridor in network[0]:
        ...
        for t in range(total_tick):
            ...
            for elem in main_roads:
                if not t % supply_period and elem.ramp_flag == 0:
                    link_order = linkdf.where(linkdf['link_id'] == elem.linkid).dropna(subset=['corridor_id']).iloc[0]['corridor_link_order']
        ...
        df.to_csv(f"Density_profile_{corridor}.csv")
        flowdf.to_csv(f"Flow_profile_{corridor}.csv")
